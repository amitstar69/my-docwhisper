...
export default function BotChat({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchBot = async () => {
      const { data } = await supabase.from('bots').select('store_name').eq('id', params.id).single();
  ...
  const sendQuery = async () => {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/query-bot`, {
      ...
      body: JSON.stringify({
        query: input,
        storeName,
        metadataFilter: '' // Add if needed
      })
    });
    const data = await res.json();
    setMessages([...messages, { user: input, ai: data.text, citations: data.citations }]);
    setInput('');
  };
